...
public class Simulation {
    ...
    public static void main(String[] args){
        ...
        Automail automail = new Automail(new ReportDelivery(), false);
        ...
        while(MAIL_DELIVERED.size() != MAIL_TO_CREATE) {
        	//System.out.println("Step: "+Clock.Time()+", delivered: "+MAIL_DELIVERED.size());
            generator.step();
            automail.robot.step();
            Clock.Tick();
        }
        ...
        Automail automail2 = new Automail(new ReportDelivery(), true);

        MailGenerator generator2 = new MailGenerator(MAIL_TO_CREATE, automail2.mailPool, seedMap);

        /** Initiate all the mail */
        generator2.generateAllMail();

        while(MAIL_DELIVERED.size() != MAIL_TO_CREATE) {
            //System.out.println("Step: "+Clock.Time()+", delivered: "+MAIL_DELIVERED.size());
            generator2.step();
            automail2.robot.step();
            Clock.Tick();
        }
